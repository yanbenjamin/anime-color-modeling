...
OUTPUT_CHANNELS = 2 #the A and B color channels in the LAB model 

def load_image_CIELAB(image_file):
  #print(image_file.numpy().decode("utf-8"))
  lab_img = cv2.imread(image_file) #.numpy().decode("utf-8"))
  if (lab_img.any() == None):
    raise ValueError(f"invalid image at {image_file}")

  lab_img = (lab_img - 127.5) / 127.5

  #L intensity, then a stack of A and B channels to generate for color representation
...
if __name__ == "__main__":
    ...
    fname_to_save_to = "manga-anime-model.keras"
